...
re_applyarea = re.compile(r'\w{3}-\w{3,4}-\w{3,4}|\w{3}-\w{3,4}')  #match apply enterprise qulification area by crawl entpq
...
def applyarea_filter(li_applyarea):
    entqlurl_list = [element for element in li_applyarea if re_applyarea.match(element)!= None]
    return entqlurl_list